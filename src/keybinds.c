...
void keybinds_handle(xcb_key_press_event_t *ev) {
    ...
    else if (keysym == XK_q && (ev->state & XCB_MOD_MASK_SHIFT) && (ev->state & XCB_MOD_MASK_4)) exit(0);
    else if (keysym == XK_c && (ev->state & XCB_MOD_MASK_SHIFT) && (ev->state & XCB_MOD_MASK_4)) {
        if (clients) xcb_destroy_window(conn, clients->win);
    }
    xcb_key_symbols_free(syms);
}
